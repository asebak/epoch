...
-spec oracle_respond(aec_keys:pubkey(), aeo_query:id(), binary(), binary(), chain_state()) ->
    prim_op_result().
oracle_respond(Oracle, QueryId, Signature, Response, State) ->
    on_chain_only(State,
                  fun() -> oracle_respond_(Oracle, QueryId, Signature, Response, State) end).

oracle_respond_(Oracle, QueryId, Signature, Response,
               State = #state{ account = ContractKey }) ->
